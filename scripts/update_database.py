...
def get_or_create_branch(branch_name, company_name, bank_name):
    try:
        ...
        if not company:
            return None
        branch = Branch.query.filter_by(branch_name=branch_name, company_id=company.company_id).first()
        if not branch:
            branch = Branch(branch_name=branch_name, company_id=company.company_id, bank_name=bank_name)
...
def save_user(transaction):
    try:
        ...
        if not user:
            ...
            encrypted_email = cipher_suite.encrypt(transaction['user_id'].encode())
...
def save_bank_statement(transaction, user_id):
    try:
        ...
        if not company:
            return None
        branch = get_or_create_branch(transaction['branch_name'], transaction['company_name'], transaction['bank_name'])
        if not branch:
            return None
        ...
        new_statement = BankStatement(
            ...
            statement_data=f"s3://{transaction['company_name']}_{transaction['branch_name']}_transactions.csv"
        )

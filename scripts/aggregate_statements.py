...
def aggregate_and_save(transactions, company_name):
    ...
    with open(filename, 'w', newline='') as csvfile:
        fieldnames = ['transaction_id', 'statement_id', 'date', 'amount', 'description', 'company_name', 'branch_id', 'branch_name', 'bank_name']
...
def aggregate():
    try:
        ...
        print("Temporary configuration file not found. Please run the download script first.")
        return
    ...
    choice = int(input("Enter the number corresponding to the company: "))
    company_name = company_options.get(choice)
    if company_name:
        transactions = get_transactions_for_company(company_name, year, month)
